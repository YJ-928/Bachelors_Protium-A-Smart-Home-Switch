...
def control():
    ...
    if request.method == 'POST':
        ...
        if jsn_website['led1'] != jsn_server['led1']:
            ...
            w_url = webhook_url + "D5"
            if jsn_server['led1'] == "ON":
                d = ['1']
            else:
                d = ['0']
            r = requests.put(w_url, data=json.dumps(d), headers={'Content-Type':'application/json'})
        ...
        if jsn_website['led2'] != jsn_server['led2']:
            ...
            w_url = webhook_url + "D4"
            if jsn_server['led2'] == "ON":
                d = ['1']
            else:
                d = ['0']
            r = requests.put(w_url, data=json.dumps(d), headers={'Content-Type':'application/json'})
        ...
        if jsn_website['led3'] != jsn_server['led3']:
            ...
            w_url = webhook_url + "D0"
            if jsn_server['led3'] == "ON":
                d = ['1']
            else:
                d = ['0']
            r = requests.put(w_url, data=json.dumps(d), headers={'Content-Type':'application/json'})
        ...
        if jsn_website['fan'] != jsn_server['fan']:
            ...
            w_url = webhook_url + "D2"
            if jsn_server['fan'] == "ON":
                d = ['1']
            else:
                d = ['0']
            r = requests.put(w_url, data=json.dumps(d), headers={'Content-Type':'application/json'})
